...
class PaymentVerificationMixin:
    ...
    def assign_keys_and_notify(self, request, payment):
        ...
        try:
            ...
            logging.error(f"Error during key assignment and notification: {e}")
            messages.error(
                request,
    ...
    def assign_unique_keys_to_order(self, order_id):
        ...
        for order_item in order.order_items.all():
            ...
            self._assign_keys_and_update_stock(
                order_item, available_keys, quantity
            )

    def _assign_keys_and_update_stock(
            self, order_item, available_keys, quantity
    ):
        """
        ...
        product.quantity_in_stock = max(
            0, product.quantity_in_stock - quantity
        )
    ...
    def notify_site_owner(self, request, payment):
        ...
        message = f"User {user.username} has made a new purchase. Order details are attached."  # noqa
