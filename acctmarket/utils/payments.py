...
class NowPayment:
    ...
    def __init__(
        self, callback_url_name="referals:nowpayment_callback",
...
class Flutterwave:
    ...
    def __init__(self):
        ...
        self.secret_key = (
            settings.FLUTTERWAVE_PRIVATE_KEY if settings.USE_FLUTTER_WAVE_TESTING   # noqa
            else settings.FLUTTERWAVE_SECRET_KEY
        )
        self.public_key = (
            settings.FLUTTERWAVE_PUBLIC_KEY_TEST if settings.USE_FLUTTER_WAVE_TESTING   # noqa
    ...
    def verify_payment(self, transaction_id, max_retries=3):
        ...
        for attempt in range(max_retries):
            try:
                response = requests.get(
                    verify_url, headers=self.headers
                )
                response_data = response.json()
                logger.info(
                ...
                if response.status_code == 200 and response_data["status"] == "success" and response_data["data"]["status"] == "successful":  # noqa
                    return {
                        "status": "success",
                        "amount": response_data["data"]["amount"]
                    }

                if response_data.get("message") == "No transaction was found for this id": # noqa
                    logger.warning("Transaction not found on Flutterwave - retrying.")   # noqa
                ...
                logger.error(f"Network error during verification for transaction {transaction_id}: {e}")  # noqa

        logger.error(f"Verification failed after {max_retries} attempts for transaction {transaction_id}")  # noqa
        return {
            "status": "error",
