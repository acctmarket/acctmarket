...
class CustomSignupBaseForm(SignupForm):
    ...
    phone_no = CharField(max_length=20, label="Phone number", required=True)
    phone_region = ChoiceField(
        choices=[(country.alpha_2, country.name) for country in pycountry.countries],  # noqa
        initial="NG",
        help_text="Select your phone region code (e.g., 'NG' for Nigeria)",  # noqa
    )
    terms = BooleanField(
