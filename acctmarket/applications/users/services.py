...
class TwilloSMSService:
    ...
    def send_otp_via_sms(self, user: User):
        ...
        otp = user.generate_otp()  # Call the OTP generation method on the user model  # noqa
    ...
    def send_bulk_sms(self, recipients, message):
        ...
        """
        if not isinstance(recipients, list) or not all(isinstance(r, str) for r in recipients):  # noqa
            logger.error(
                "Recipients must be a list of phone numbers."
            )
            raise ValueError(
        ...
        for recipient in recipients:
            ...
            logger.info(f"Bulk SMS process completed. Sent to {len(recipients)} recipients.")  # noqa
        return results
