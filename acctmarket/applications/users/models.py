...
class User(UIDTimeBasedModel, AbstractUser):
    ...
    def formatted_phone_number(self):
        ...
        try:
            ...
            logger.info(
                f"Parsing phone_no: {self.phone_no}, phone_region: {self.phone_region}"  # noqa
            )
            parsed_number = phonenumbers.parse(
                self.phone_no, self.phone_region
            )
            if phonenumbers.is_valid_number(parsed_number):
                ...
                logger.info(f"Valid phone number formatted: {formatted}")
                return formatted
            logger.warning(f"Invalid phone number: {self.phone_no}")
            return None
        except NumberParseException as e:
            logger.error(f"NumberParseException for {self.phone_no}: {e}")
            return None
