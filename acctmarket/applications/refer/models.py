...
from acctmarket.applications.refer.managers import ReferralManager
...
class Referral(TimeBasedModel):
    ...
    referred_user = auto_prefetch.ForeignKey(
        ...
        on_delete=models.SET_NULL, related_name="refered_by"
    )
    ...
    def __str__(self):
        return f"Referral by {self.referrer.username}"  # noqa
    ...
    def apply_first_purchase_reward(self, purchase_amount):
        ...
        try:
            ...
            logger.info(f"getting the referrers wallet for first purchase {referrer_wallet}")   # noqa
        except (Customer.DoesNotExist, Wallet.DoesNotExist):
            # Handle the case where no Customer profile exists for the referrer
            raise ValueError(f"No Customer profile or wallet found for user {self.referrer}")  # noqa
        tier_commission_map = {
        ...
        tier_rate = tier_commission_map.get(
            referrer_customer.tier, Decimal("0.07")
        )
        ...
        self.save(update_fields=[
            "first_purchase_done", "is_completed", "first_purchase_amount"
        ])
...
class Wallet(TimeBasedModel):
    ...
    def credit_wallet(self, amount):
        ...
        logger.info(f"Crediting wallet of user {self.user.username} with amount {amount}.")  # noqa
        ...
        self.record_transaction(
            amount, WalletTransactionTypeChoice.CREDIT
        )
    ...
    def record_transaction(self, amount, transaction_type):
        ...
        logger.info(f"Recording {transaction_type} transaction for user {self.user.name} with amount {amount}.")  # noqa
        WalletTransaction.record_transaction(
            self, amount, transaction_type
        )
...
class WalletTransaction(TimeBasedModel):
    ...
    wallet = auto_prefetch.ForeignKey(
        "refer.Wallet", on_delete=models.CASCADE,
        related_name="transactions"
    )
    amount = models.DecimalField(max_digits=10, decimal_places=2)
    transaction_type = models.CharField(
        max_length=6, choices=WalletTransactionTypeChoice.choices,
...
class Notification(TimeBasedModel):
    ...
    notification_type = models.CharField(
        max_length=20, choices=NOTIFICATION_TYPES_Choice.choices
    )
...
class SMSCampaign(TimeBasedModel):
    ...
    def send_to_users(self):
        ...
        if results:
            ...
            logger.error(
                f"Campaign '{self.name}' failed to send any messages."
            )
