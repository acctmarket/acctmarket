...
class Category(ImageTitleTimeBaseModels):
  ...
  # Match your custom PK length  # noqa
...
class Product(TitleandUIDTimeBasedModel, ImageTitleTimeBaseModels):
    ...
    def get_applicable_discount(self):
        ...
        best_coupon = max(
            coupons,
            key=lambda coupon: (
                coupon.discount_value if coupon.discount_type == "flat" else
                (self.price * coupon.discount_value / 100)
            )
        )
...
class Payment(TimeBasedModel):
    ...
    wallet = auto_prefetch.ForeignKey(
        "refer.Wallet", on_delete=CASCADE,
    ...
    def verify_flutterwave_payment(self) -> bool:
        ...
        try:
            ...
            expected_amount = convert_to_naira(self.amount, exchange_rate)  # noqa
            ...
            if result["status"] == "success":
                ...
                logging.error(f"Verification failed for payment reference {self.reference}: " # noqa
                            f"status {result['status']}, message {result.get('message')}.") # noqa
...
class Coupon(TimeBasedModel):
    ...
    discount_type = CharField(
        max_length=10, choices=COUPON_CHOICE.choices,
    ...
    valid_from = DateField(
        null=True, blank=True, help_text="Start date of coupon validity."
    )
    valid_to = DateField(
    ...
    def __str__(self):
        return f"{self.code} ({self.get_discount_type_display()}: {self.discount_value})"  # noqa
